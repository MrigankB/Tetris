...
public class TBlock implements Block
{
	...
	public TBlock(Point location)
	{
		...
		points[1] = new Point(location.getX() + 1, location.getY());
		points[2] = new Point(location.getX() - 1, location.getY());
	}
		...
		switch(this.orientation)
		{
			...
			case RIGHT:
				return points[0].getX() == f.getColumnLimit() ||
						f.isFilled(points[1].getX() + 1, points[1].getY()) ||
						f.isFilled(points[0].getX() + 1, points[0].getY()) ||
						f.isFilled(points[2].getX() + 1, points[0].getY());
			case DOWN:
		...
		switch(this.orientation)
		{
			case UP:
				return points[2].getX() == 0 ||
						f.isFilled(points[0].getX() - 1, points[0].getY()) ||
						f.isFilled(points[2].getX() - 1, points[1].getY());
			case RIGHT:
